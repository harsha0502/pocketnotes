...
const NoteHeader = ({ name, color, isMobile, setDisplay }) => {
  // Extract initials (First letter of the first two words)
  const initials = name
    .split(" ")
  ...
  return (
    <div 
      className="note-header flex flex-row justify-start"
      // CRITICAL CHANGE: Apply the group color to the header background 
      // via a CSS Variable, as per the CSS changes I provided earlier.
      style={{ '--header-bg-color': color }} 
    >
      {isMobile && (
        <svg
      ...
      <div
        ...
        // Apply the color prop to the icon background
        style={{ marginRight: "1.5rem", backgroundColor: color, color: 'white' }} 
      >
        {/* Use the calculated initials */}
        {initials}
      </div>
